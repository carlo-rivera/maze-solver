...
class Cell():
    ...
    def draw(self, x1, y1, x2, y2):
        if self._win is None:
            raise Exception("tried to draw() without window")
    ...
    def draw_move(self, to_cell, undo=False):
        if self._win is None:
            raise Exception("tried to draw_move() without window")
...
class Maze():
    ...
    def _draw_cell(self, i, j):
        if self._win is None:
            raise Exception("tried to _draw_cell() without window")
    ...
    def _animate(self):
        if self._win is None:
            raise Exception("tried to _animate() without window")
    ...
    def _solve_r(self, i, j):
        ...
        if i < self._num_cols - 1 and not self._cells[i + 1][j].visited and not self._cells[i][j].has_right_wall:
            self._cells[i][j].draw_move(self._cells[i + 1][j], j)
            if self._solve_r(i + 1, j):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i + 1][j], True)
